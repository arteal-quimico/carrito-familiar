...
export default function PedirView({ products, pending, onAddPending, onRemovePending, onOpenCreate, onEditProduct }) {
  ...
  const getQty = (catId, productId) => pending[getKey(catId, productId)]?.pendingQty || 0

  const changeQty = (catId, product, delta) => {
    const key    = getKey(catId, product.id)
    const newQty = Math.max(0, getQty(catId, product.id) + delta)
    if (newQty === 0) {
      onRemovePending(key)
    } else {
      onAddPending(key, {
  ...
  return (
    <div className="pedir-view">
      ...
      <div className="categories-scroll">
        {CATEGORIES.map(cat => (
          <button key={cat.id} className={`cat-pill ${cat.id === selectedCat ? 'selected' : ''}`} onClick={() => setSelectedCat(cat.id)}>
        ...
        {prods.map(product => {
          ...
          return (
            <div key={product.id} className={`product-card ${isAdded ? 'added' : ''}`}>
              <button className="edit-btn" onClick={e => { e.stopPropagation(); onEditProduct({ ...product, category: selectedCat }) }}>✏️</button>
